...
void handlePlayerInput(SDL_Rect *playerRect, int *playerX, int *playerY, int *playerVelocityX, int *playerVelocityY, int up, int down, int left, int right, int levelWidth, int levelHeight, int playerRectWidth, int playerRectHeight, int speed) {
    ...
    *playerY += *playerVelocityY / 60;
    if ((*playerX < 0) || (*playerX + playerRect->w > levelWidth)) 
    {
        *playerX -= *playerVelocityX;
    }
    if ((*playerY < 0) || (*playerY + playerRect->h > levelHeight))
    {
        *playerY -= *playerVelocityY;
    }
    if (*playerX > levelWidth - playerRectWidth) *playerX = levelWidth - playerRectWidth;
    if (*playerY > levelHeight - playerRectHeight) *playerY = levelHeight - playerRectHeight;
